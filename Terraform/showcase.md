Below is a single-file **Terraform language showcase** that covers the major **syntax, concepts, and built-in features** in one place: blocks, arguments, expressions, variables, locals, outputs, resources, data sources, meta-arguments, lifecycle rules, validations, checks, provisioners, templating, functions, `for_each`, `count`, and more. Terraform’s configuration model is built around **blocks, arguments, and expressions**, with additional top-level structures like `terraform`, `provider`, `variable`, `data`, `resource`, `module`, and `output`, plus meta-arguments such as `depends_on`, `count`, `for_each`, and `lifecycle`. ([HashiCorp Developer][1])
...
* This is a **broad showcase**, not literally every Terraform feature ever added.
* Some concepts are intentionally **commented examples** because they depend on real external infrastructure, separate modules, or state refactoring history.
* The file is still useful as a study/reference sheet and can be trimmed into a runnable demo.

I can also turn this into a **cleaner interview-prep version** or a **production-style Terraform skeleton**.

[1]: https://developer.hashicorp.com/terraform/language?utm_source=chatgpt.com "Overview - Configuration Language | Terraform"
